...
//__device__ void setup_rotation(const float *ref_xlist, const float *mov_xlist, int& n_list, float R[3][3], float* E0, int& i, int& j, int& n)
//__device__ void setup_rotation(const float *ref_xlist, const float *mov_xlist, int& n_list, float R[3][3], float* E0)
{
  //int i, j, n;
...
__device__ int jacobi3(float a[3][3], float d[3], float v[3][3], volatile int& i, volatile int& j, volatile int& k, volatile int& n)
//__device__ int jacobi3(float a[3][3], float d[3], float v[3][3], int& i, int& j, int& k, int& n)
//__device__ int jacobi3(float a[3][3], float d[3], float v[3][3])
{
  //volatile int n, k, i, j;
  //int n, k, i, j;
  ...
  for (n=0; n<50; n++)     
  {
    ...
    for (i=0; i<2; i++) 
    {
      for (j=i+1; j<3; j++) 
      {
        ...
        if ( n > 3  &&  fabsf(d[i])+g == fabsf(d[i]) &&  fabsf(d[j])+g == fabsf(d[j]) ) 
        {
          a[i][j] = 0.0;
        } 
 ...
 */
__device__ int diagonalize_symmetric(float matrix[3][3], float vec[3][3], float eigenval[3], volatile int& i, volatile int& j, volatile int& k, volatile int& n)
//__device__ int diagonalize_symmetric(float matrix[3][3], float vec[3][3], float eigenval[3], int& i, int& j, int& k, int& n)
//__device__ int diagonalize_symmetric(float matrix[3][3], float vec[3][3], float eigenval[3])
{
  //int n_rot, i, j, k;
  //volatile int i, j, k;
  //float vec[3][3];
  ...
  if (!jacobi3(matrix, eigenval, vec, i, j, k, n))
 ...
 */
__device__ int calculate_rotation_matrix(float R[3][3], float& E0, float* residual, volatile int& i, volatile int& j, volatile int& k, volatile int& n)
//__device__ int calculate_rotation_matrix(float R[3][3], float& E0, float* residual, int& i, int& j, int& k, int& n)
//__device__ int calculate_rotation_matrix(float R[3][3], float& E0, float* residual)
{
	//int i, j, k;
  ...
  if (!diagonalize_symmetric(RtR, right_eigenvec, eigenval, i, j, k, n))
...
__global__ void gRmsdMaxSuper(int nato, const int nframes, const float* gclust_coords, const int cluster, const float cutoff, const int* frameapp_read, int* frameapp_write, float* distance) {
	...
	//volatile unsigned int index = blockIdx.x * blockDim.x + threadIdx.x;	
	...
	if(frameapp_read[cluster] == -1) {
		...
		setup_rotation(&gclust_coords[cluster*3*nato],&gclust_coords[index*3*nato], nato, R, &E0,i,j,n);
		//setup_rotation(&gclust_coords[cluster*3*nato],&gclust_coords[index*3*nato], nato, R, &E0);		
			
		float rmsd=-1;
		if(calculate_rotation_matrix(R, E0, &rmsd,i,j,k,n)) {
